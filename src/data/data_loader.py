...
def load_engineered_data(path: str | Path | None = None) -> pd.DataFrame:
    ...
    csv_path = Path(path) if path is not None else DEFAULT_ENGINEERED_DATA_PATH
    if not csv_path.is_absolute():
        csv_path = PROJECT_ROOT / csv_path

    if not csv_path.exists():
        raise FileNotFoundError(f"Engineered dataset not found at: {csv_path}")
...
def load_selected_data(path: str | Path | None = None) -> pd.DataFrame:
    """Load the final selected Telco churn dataset from disk."""
    csv_path = Path(path) if path is not None else DEFAULT_SELECTED_DATA_PATH
    if not csv_path.is_absolute():
        csv_path = PROJECT_ROOT / csv_path

    if not csv_path.exists():
        raise FileNotFoundError(f"Selected dataset not found at: {csv_path}")
